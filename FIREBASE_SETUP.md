...
The original project, `langlens-509100`, could not attach billing because of a project quota. The backend moved to the already-billed `halogen-bonus-4phrv` project, preserving the Google account UID, existing cloud records and revision. Old cloud data is retained with client access disabled so stale tabs cannot sync against the retired backend.

Live verification on 2026-09-19: the original browser signed in to the new project and completed uploads of all 144 study items, 21 sources, 27 reading notes/annotations, and 7 attachments. An independent restore used the actual CloudSync logic with a fresh in-memory IndexedDB database and authenticated administrative API adapters to read the live cloud data. It restored all records and downloaded all 7 files (11,575,940 bytes), verified each file's SHA-256 hash, checked linked records, and confirmed no pending changes. This verifies the restore logic and stored files; the automated restore uses administrative access rather than a second browser's Google session. Owner/other-user/unauthenticated access is covered separately by the rule evaluations. The bucket has no public IAM grants.
